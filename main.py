...
class VarShapeLearner(object):
    ...
    def _weights_init(self):
        ...
        self.enc_conv0, self.enc_conv1, self.enc_conv2, self.enc_conv3 = ([0] * (self.local_latent_num + 1) for _ in range(4))
        ...
        self.weights_all['W'] = {
            ...
            'enc_conv0': [tf.get_variable(name='enc_conv0', shape=[12, 12, 12, 1, 16],
                                          initializer=layers.xavier_initializer())]*(self.local_latent_num+1),
            'enc_conv1': [tf.get_variable(name='enc_conv1', shape=[6, 6, 6, 16, 32],
                                          initializer=layers.xavier_initializer())]*(self.local_latent_num+1),
            ...
            'dec_conv2': tf.get_variable(name='dec_conv2', shape=[5, 5, 5, 32, 64],
                                          initializer=layers.xavier_initializer()),
            'dec_conv3': tf.get_variable(name='dec_conv3', shape=[6, 6, 6, 16, 32],
                                          initializer=layers.xavier_initializer()),
            'dec_conv4': tf.get_variable(name='dec_conv4', shape=[12, 12, 12, 1, 16],
                                          initializer=layers.xavier_initializer())
        }

        self.weights_all['b'] = {
            # input_shape x -> all_lat z_0, z_i (0, 1:local_lat_num)
            'enc_conv0' : [tf.Variable(name='enc_conv0', initial_value=tf.zeros(16))]*(self.local_latent_num+1),
            ...
            'dec_conv3': tf.Variable(name='dec_conv3', initial_value=tf.zeros(16)),
            'dec_conv4': tf.Variable(name='dec_conv4', initial_value=tf.zeros(1)),
        }
    ...
    def _inf_model(self, weights, biases):
        ...
        for i in range(self.local_latent_num + 1):
            self.enc_conv0[i] = tf.nn.relu(tf.nn.conv3d(self.x, weights['enc_conv0'][i],
                                                        strides=[1, 1, 1, 1, 1], padding='VALID')
                                                        + biases['enc_conv0'][i])
            self.enc_conv1[i] = tf.nn.relu(tf.nn.conv3d(self.enc_conv0[i], weights['enc_conv1'][i],
    ...
    def _gen_model(self, weights, biases):
        ...
        dec_conv3    = tf.nn.relu(tf.nn.conv3d_transpose(dec_conv2, weights['dec_conv3'],
                                     output_shape=[self.batch_size, 30, 30, 30, 16],
                                     strides=[1, 2, 2, 2, 1], padding='VALID') + biases['dec_conv3'])
        dec_conv4    = tf.nn.sigmoid(tf.nn.conv3d_transpose(dec_conv3, weights['dec_conv4'],
                                     output_shape=[self.batch_size, 41, 41, 41, 1],
                                     strides=[1, 1, 1, 1, 1], padding='VALID') + biases['dec_conv4'])
        return dec_conv4
    ...
    def _model_loss_optimizer(self):
        ...
        # self.rec_loss = tf.losses.mean_squared_error(self.x, self.x_rec)
...
# data = sio.loadmat('shapenet_airplane_vox_30_split.mat')

# data = np.load('data/filterTrianData.npy')

train_all = np.load('data/filterTrianData.npy')
test_all  = np.load('data/testData.npy')
...
VSL.saver.restore(VSL.sess, os.path.abspath('parameters/modelnet40-0325-cost-2.2855.ckpt'))
...
for epoch in range(total_epoch):
    ...
    index = epoch + 325 # correct the training index, set 0 for training from scratch

    # iterate for all batches
    np.random.shuffle(train_all)
    for i in range(train_batch):
        x_train = train_all[batch_size*i:batch_size*(i+1),0:].reshape([batch_size, obj_res, obj_res, obj_res, 1])
